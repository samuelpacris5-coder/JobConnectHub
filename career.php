...
if($_GET['delete'])
		  {
		          mysql_query("delete from careers where id = '".$_GET['delete']."'");
...
popUpWin = open(URLStr,'popUpWin', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,copyhistory=yes,width='+600+',height='+500+',left='+left+', top='+top+',screenX='+left+',screenY='+top+'');
}
...
<!--header-->
   ...
   <!--left-->
     ...
     <table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td width="1%">&nbsp;</td>
        <td width="99%" class="heading5">Manage Careers</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
		 <? 
			  $find=$_POST['search'];
$str = mysql_query("SELECT *FROM careers WHERE name LIKE'%$find%' || email LIKE'%$find%' || Contact_no LIKE'%$find%'   ");
...
?>
	  ...
	  <? echo $_SESSION['del']; ?><? echo $_SESSION['del']="";?></font></p>
          ...
          <?php 
			 ...
			 $ret = mysql_query("SELECT *FROM careers WHERE name LIKE'%$find%' || email LIKE'%$find%' || Contact_no LIKE'%$find%'   limit ".$start.",".$noofrecord."");
			$cnt=1;
          ...
          <tr >
            ...
            <td style=" float:left;"   class="comb-value"><?php echo $cnt; ?>.</td>
              ...
              <td  class="comb-value"><? echo $row['Name'];?></td>
               <td  class="comb-value"><? echo $row['Email'];?></td>
			    <td  class="comb-value"><? echo $row['Contact_no'];?></td>
               <td  class="comb-value"><? echo $row['Resume'];?></td>
			    <td width="7%" align="left"><a href="javascript:void(0);"  class="comb-value" onClick="popUpWindow('profile.php?id=<?php echo $row['id'];?>');"><img src="images/view.png" alt="View this Profile" style="border:0px;"></a></td>
            <td width="7%" class="comb-value"><a href="career.php?delete=<?php echo $row['id'];?>" class="comb-value" onClick="return delete(); ">   <img src="images/delete.png" alt=""  style="border:0px"; ></a></td>
          </tr>
	...
	if ($anymatches == 0) 
		...
		echo "<b><font size = 2px color = red>" .$find;  echo "</b></font>" ;
          ...
          <tr>
			...
			<?php list($totalrecord)=mysql_fetch_array(mysql_query("select count(*) as tt FROM careers WHERE name LIKE'%$find%' || email LIKE'%$find%' || Contact_no LIKE'%$find%'  "));
		...
		echo '<a href="career.php?search='.urlencode($find).'&page='.$i.'">'.($i+1).'</a> | ';
		}
					?></td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
